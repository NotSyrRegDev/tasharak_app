...
const AddProductRating = ({ navigation , route }) => {
  ...
  const {   isLoading, error , success  , addProductRating} = useContext(AppContext);

  const [userId, setUserId] = useState('');
  const [fullName , setFullName] = useState('');
  const [email , setEmail] = useState('');
  const [findProductAsDescribed , setFindProductAsDescribed] = useState(true);
  const [ratingeHeadline , setRatingeHeadline] = useState('');
  ...
  const handleRating = (value) => {
    ...
    const handleAddRating = () => {
      addProductRating(userId , fullName , email   , ratingeHeadline , ratingeDesc , checkRemeber , route.params.product_id );
...
<View  className="flex flex-row items-center justify-between mt-5 mb-8" style={styles.orderSummaryDiv} >

<View style={styles.orderInfo} >
  <Text style={styles.font} className="text-base text-left w-52 text-black" > {route.params.product_name}   </Text>

  <View style={styles.productCateogry} className="rounded-full  px-3 py-1 mt-4" >
  <Text style={styles.font} className="text-xs  text-white" > {route.params.product_category} </Text>
  </View>

</View>
<View style={styles.orderActions} className >
  <Image className="h-32 w-full rounded-lg"  source={{ uri: route.params.product_image }} />
...
<View className="mb-12">
      ...
      <View>
      <Text style={styles.font} className="text-base block text-gray-700 font-bold  text-left" htmlFor="username">
      أكتب تقييم المنتج   
      </Text>
      ...
      <Text style={styles.textInput} className="block text-gray-700 font-bold mb-2" htmlFor="username">
      أكتب عنوان   <Text className="text-red-500 text-base" > * </Text>  
      </Text>
      <TextInput
        style={styles.inputStyle}
        className="appearance-none border rounded-xl w-full py-4 px-3 text-gray-700 leading-tight"
        id="username"
        placeholder="مالذي تريد تسليط الضوء عليه؟"
        value={ratingeHeadline}
        onChangeText={(text) => setRatingeHeadline(text)}
        />
      </View>
      ...
      <Text style={styles.textInput} className="block text-gray-700 font-bold mb-2" htmlFor="username">
      أضف تقييم  <Text className="text-red-500 text-base" > * </Text>  
      </Text>
      <TextInput
