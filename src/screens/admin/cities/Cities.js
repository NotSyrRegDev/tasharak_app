     ...
     المدن الحالية
      ...
      {citiesArray && citiesArray.length !== 0 ? citiesArray.map((item , index) => (
        ...
        className="mt-3 text-black py-3 bg-gray-800 hover:bg-gray-900 rounded-lg text-sm px-6  mb-2 w-full"
          style={styles.button}
          onPress={() => navigation.navigate('AddCityDashboard')}>
          <Text style={styles.buttonText}> إضافة مدينة </Text>
        </TouchableOpacity>
...
<View style={styles.modalContainer} >
  ...
  {error && (
        ...
        {success && (
          ...
          <Text className="text-black text-center text-lg mb-5" style={styles.font} > الاجرائات على المدينة  </Text>
    ...
    editId : editId,
      ...
      <TouchableOpacity
        className="mt-2 text-center text-black py-3 bg-red-400  rounded-lg text-sm px-6  mb-2 w-full"
          style={styles.buttonBG}
          onPress={() => {
            deleteRecord("cities" , editId);
          } }>
          <Text style={styles.buttonText}>   حذف المدينة   </Text>
        </TouchableOpacity>
