...
const ManageExperience = ({navigation , route}) => {
  ...
  useEffect(() => {
    const getInfoFromFireStore = async () => {
      const docRef = doc(db, "experiences", route.params.movieid );
  ...
  return (
    ...
    {movieData?.exp_name}  
        ...
        onPress={() => navigation.navigate('EditExperienceGuides' , {
          eventId: movieData?.id,
          eventName: movieData?.exp_name
        }) }
        >
          ...
          eventName: movieData?.exp_name
            ...
            <TouchableOpacity className="w-1/4 flex-col items-center" style={styles.boxManage}
              onPress={() => navigation.navigate('EditExperienceDashboard' , {
                eventId: movieData?.id,
          eventName: movieData?.exp_name
              } ) }
             >
            ...
            <TouchableOpacity className="w-1/4 flex-col items-center" style={styles.boxManage}
            onPress={() => navigation.navigate('EditExperienceAttendance' , {
              eventId: movieData?.id,
              eventName: movieData?.exp_name
            } ) }
             >
            <Image source={require('../../../assets/icons/admin_icons/about.png')} style={styles.iconBox} />
            <Text style={styles.font} className="text-black text-center mt-5 text-base" > الحضور </Text>
            </TouchableOpacity>

            <TouchableOpacity className="w-1/4 flex-col items-center" style={styles.boxManage}
            onPress={() => navigation.navigate('EditExperienceHost' , {
              eventId: movieData?.id,
              eventName: movieData?.exp_name
            } ) }
             >
            <Image source={require('../../../assets/icons/admin_icons/host.png')} style={styles.iconBox} />
            <Text style={styles.font} className="text-black text-center mt-5 text-base" > الاستضافة </Text>
            </TouchableOpacity>

            <TouchableOpacity className="w-1/4 flex-col items-center" style={styles.boxManage}
            onPress={() => navigation.navigate('EditExperiencePhotos' , {
              eventId: movieData?.id,
              eventName: movieData?.exp_name
            } ) }
             >
    ...
    <Text style={styles.font} className="block text-black font-bold mb-2 text-2xl"  >
        ...
        />
          ...
          <View className="p-3 text-sm text-black rounded-lg  text-right mb-5 flex flex-row items-center" >
          <Text style={styles.errorText} className="mx-1"  >   {movieData?.exp_rating}  </Text>
          <AntDesign
          name="staro"
          size={26}
          style={[styles.starIcon]}
        />
          ...
          <View className="p-3 text-sm text-black rounded-lg  text-right mb-5 flex flex-row items-center" >
          <Text style={styles.errorText} className="mx-2"  >   {convertTimeToDateString(movieData?.exp_date)}  </Text>
          <Fontisto
          name="date"
          size={26}
          style={[styles.starIcon]}
