...
const AddDeliveryRating = ({ navigation , route }) => {
  ...
  const {   isLoading, error , success  , addDeliveryRating} = useContext(AppContext);

  const [userId, setUserId] = useState('');
  const [fullName , setFullName] = useState('');
  const [email , setEmail] = useState('');
  const [findProductAsDescribed , setFindProductAsDescribed] = useState(true);
  const [ratingLike , setRatingLike] = useState('');
  const [ratingDisLike , setRatingDisLike] = useState('');
  ...
  const handleRating = (value) => {
    ...
    const handleAddRating = () => {
      addDeliveryRating(userId , fullName , email , ratingLike ,  ratingDisLike ,  checkRemeber , route.params.product_id );
...
<View className="mb-12">
      ...
      <View className="mb-8 mt-8">
        ...
        id="username"
        placeholder="قولنا أكثر"
        value={ratingLike}
        onChangeText={(text) => setRatingLike(text)}
        />
        </View>
       ...
       ما الذي لم يعجبك ؟
        ...
        placeholder="قولنا أكثر"
        value={ratingDisLike}
        onChangeText={(text) => setRatingDisLike(text)}
        />
        </View>
...
export default AddDeliveryRating