...
const handleBookingProduct = () => {
  ...
  return (
    <SafeAreaView style={styles.container} >
      ...
      { /* TOP HEADER TEXT */ }
    <TopProfileNavigator navigation={navigation} text={"تفاصيل الغرض"} />
...
<Text className="text-base text-black text-left" style={styles.font} > {productData.productAdditional[0].maxRentalPrice} أيام </Text>
  <Text className="text-sm mt-1 text-left " style={[styles.font , styles.price]} > الحد الأعلى لمدة الإيجار  </Text>
      ...
      {productData.product_tags !== 0 && productData.product_tags.map((item , index) => (
        <TouchableOpacity key={index} className="rounded-full mx-1 w-24 text-center mt-2" style={styles.button} >
      ...
      ref={mapRef} 
      style={styles.map}
      initialRegion={productData?.product_location}
      >
      <Marker
      draggable
      pinColor="#22BC9F"
...
<Marker
        ...
        pinColor="#22BC9F"
        coordinate={{
    longitude: productData.product_location.latitude,
    latitude: productData.product_location.longitude
  }}
        tooltip={true}
      >
        <Callout>
